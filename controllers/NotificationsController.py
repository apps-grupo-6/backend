...
@bp.post("/")
@ServerUtils.configure_request(description_code_map=NotificationsConfig.create_notification_code_map, method="POST")
@AuthUtils.validate_session
def create_notification():
    try:
        logger.info(f"{g.request_id} - starting create_notification")
        logger.info(f"{g.request_id} - starting mandatory fields check")

        data = request.json
        model = NotificationsModel.create_notification().load(data)
    ...
    return NotificationsService.create_notification(model=model, request_id=g.request_id)

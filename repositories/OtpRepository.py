...
def check_if_user_has_active_otp(user_id, type, request_id, errors_code_map, isError=True):
    ...
    if checked["data"] and not OtpUtils.check_token_expired(checked=checked, request_id=request_id):
        log = f"{request_id} - user already has an active otp_token with this type: {checked['data']['otp_token']}"
        if isError:
            logger.error(log)
        else:
            logger.debug(log)
...
@ServerUtils.set_final_response
def delete_otp(otp_token_id, request_id, errors_code_map):
    logger.info(f"{request_id} - deleting used otp_token id '{otp_token_id}'...")
    deleted = OtpManager.delete_otp(otp_token_id=otp_token_id,
                                    request_id=request_id)
    ...
    if not deleted["data"] == 1:
        logger.error(f"{request_id} - the otp_token_id is invalid")
        return {"flag": 0}

    logger.debug(f"{request_id} - otp_token deleted successfully")
    return {"flag": 1, "data": deleted['data']}

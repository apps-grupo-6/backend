...
def check_if_user_has_active_otp(user_id, type, request_id, errors_code_map):
    logger.info(f"{request_id} - checking if the user_id '{user_id}' already has an otp token")
    ...
    logger.info(checked)
    logger.debug(f"{request_id} - user already has an otp_token")
    return {"flag": 1, "data": checked['data']}

@ServerUtils.set_final_response
def save_otp(user_id, otp_token, type, request_id, errors_code_map):
    logger.info(f"{request_id} - saving otp_token '{otp_token}' type '{type}' for user_id '{user_id}'...")
...
def delete_otp(user_id, otp_token, type, request_id, errors_code_map):
    logger.info(f"{request_id} - deleting used otp '{otp_token}' with type '{type}' for user_id '{user_id}'...")
